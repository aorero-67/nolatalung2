(function () {
  ...
  window.Map3D = {
    ...
    init() {
      ...
      this.scene.fog = new THREE.FogExp2(0x050a1a, 0.04);
      ...
      const grid = new THREE.GridHelper(28, 28, 0xc9953a, 0x1a2040);
    ...
    _initSvgFallback(canvas, loading) {
      ...
      fallback.style.cssText = 'position:absolute;inset:0;display:flex;align-items:center;justify-content:center;perspective:1100px;background:radial-gradient(circle at center,rgba(201,149,58,0.08),transparent 58%);overflow:hidden';
      ...
      DISTRICT_SHAPES.forEach((d) => {
        ...
        poly.setAttribute('stroke', '#c9953a');
        ...
        bg.setAttribute('stroke','#c9953a'); bg.setAttribute('stroke-width','1.5');
        ...
        g.addEventListener('mouseenter', () => {
          poly.style.filter = 'drop-shadow(0 0 12px rgba(201,149,58,0.95))';
        ...
        g.addEventListener('click', () => {
          ...
          poly.style.filter = 'drop-shadow(0 0 18px rgba(201,149,58,1))';
    ...
    _updateDomLabels() {
      ...
      if (!this._labelDivs) {
        ...
        this.labels.forEach((anchor) => {
          ...
          div.style.cssText = 'position:absolute;pointer-events:none;white-space:nowrap;font-family:Sarabun,sans-serif;font-weight:700;font-size:11px;color:#fff;letter-spacing:0.03em;transform:translate(-50%,-50%);transition:opacity .25s, box-shadow .25s, color .25s, font-size .25s;';
    ...
    _styleMesh(mesh, active) {
      ...
      if (this._labelDivs) {
        ...
        if (entry) {
          entry.div.style.color = active ? '#ffd280' : '#fff';
          entry.div.style.fontSize = active ? '13px' : '11px';
          entry.div.style.boxShadow = active
            ? '0 0 20px rgba(201,149,58,0.7), 0 4px 16px rgba(0,0,0,0.4)'
    ...
    setDistrictHighlights(ids, primaryId) {
      ...
      this.meshes.forEach((mesh) => {
        ...
        if (mesh.userData.edges) {
          mesh.userData.edges.material.opacity = inRoute ? (isPrimary ? 1 : 0.65) : 0.12;
        }
      });
      if (primary) {
